...
from helix_apps.crispr_sandbox.services import apply_session_snapshot, build_session_snapshot
from helix_apps.med_prep.streamlit_ui import render_medprep

# ───────────────────────────────────────────────────────────────────────────────
# Streamlit page setup must be early
# ───────────────────────────────────────────────────────────────────────────────
st.set_page_config(layout="wide", page_title="Helix — Genetics Suite", page_icon="🧬")
from helix_desktop.theme import inject_helix_theme
inject_helix_theme()
...
from helix_core.db import init_db, list_sessions, load_session, save_session
...
from components.session import render_session_controls
...
# ───────────────────────────────────────────────────────────────────────────────
try:
    with open("progress.json") as _pf:
        _prog = json.load(_pf)
except Exception:
    _prog = {"xp": 0, "level": 1}
_xp = _prog.get("xp", 0)
_level = _prog.get("level", 1)
_sandbox_pill = (
    'background:#EF9F27;color:#151821;font-weight:700'
    if mode == "sandbox" else
    'background:transparent;color:#8899aa'
)
_medprep_pill = (
    'background:#EF9F27;color:#151821;font-weight:700'
    if mode == "medprep" else
    'background:transparent;color:#8899aa'
)
st.markdown(
    f'<div style="background:#151821;border-bottom:1px solid #1e2130;'
    f'padding:10px 24px;display:flex;align-items:center;'
    f'justify-content:space-between;margin-bottom:16px">'
    f'<div style="display:flex;align-items:center;gap:10px">'
    f'<div style="width:28px;height:28px;background:#1D9E75;border-radius:50%;'
    f'display:flex;align-items:center;justify-content:center;'
    f'font-size:13px;font-weight:bold;color:#fff">H</div>'
    f'<span style="color:#ffffff;font-size:14px;font-weight:600;'
    f'font-family:sans-serif">Helix</span>'
    f'</div>'
    f'<div style="display:flex;gap:6px">'
    f'<a href="?mode=sandbox" style="text-decoration:none">'
    f'<span style="{_sandbox_pill};padding:5px 16px;border-radius:20px;'
    f'font-size:13px;font-family:sans-serif">Sandbox</span></a>'
    f'<a href="?mode=medprep" style="text-decoration:none">'
    f'<span style="{_medprep_pill};padding:5px 16px;border-radius:20px;'
    f'font-size:13px;font-family:sans-serif">Med-Prep</span></a>'
    f'</div>'
    f'<div style="background:#1a1f2e;color:#1D9E75;padding:5px 16px;'
    f'border-radius:20px;font-size:13px;font-family:monospace;font-weight:600">'
    f'Lv {_level} · {_xp} XP</div>'
    f'</div>',
    unsafe_allow_html=True,
)
...
# Sidebar
# ───────────────────────────────────────────────────────────────────────────────
_sidebar = render_sidebar(seq)
enzyme = _sidebar["enzyme"]
pam_pattern = _sidebar["pam_pattern"]
pam_side = _sidebar["pam_side"]
guide_len = _sidebar["guide_len"]
scan_reverse = _sidebar["scan_reverse"]
hyper = _sidebar["hyper"]
start_pos = _sidebar["start_pos"]
end_pos = _sidebar["end_pos"]
...
def _stat_card(value: str, label: str, color: str) -> str:
    return (
        f'<div style="flex:1;background:#151821;border:0.5px solid #1e2130;'
        f'border-radius:8px;padding:16px 20px">'
        f'<div style="color:{color};font-size:1.5em;font-weight:700;'
        f'font-family:monospace">{value}</div>'
        f'<div style="color:#5F5E5A;font-size:0.75em;margin-top:4px">{label}</div>'
        f'</div>'
    )

st.markdown(
    '<div style="display:flex;gap:12px;margin:16px 0">'
    + _stat_card(str(len(pam_sites_all)), f"PAMs ({pam_pattern})", "#1D9E75")
    + _stat_card(str(len(grnas_all)), "gRNAs", "#EF9F27")
    + _stat_card(f"{len(seq):,}", "Length (bp)", "#ffffff")
    + _stat_card(str(len(off_targets)), "Off-targets", "#1D9E75")
    + '</div>',
    unsafe_allow_html=True,
)
...
render_session_controls(username, _snapshot_state)
...
# Top-level tabs
# ───────────────────────────────────────────────────────────────────────────────
tabtutorial, tabDNA, tabRNA, tabProtein, tabSonify, tabAI = st.tabs([
    "📘 Tutorial",
    "🧬 DNA",
    "🧬 RNA Tools",
    "🧪 Protein",
...
with tabDNA:
    ...
    (tabfiles, tab2d, tab3d, tab3x, tabmotif,
     tabtrans, tabmut, tabprimer, tabofftarget, tabscreen,
    ...
    with tab2d:
        ...
        _ann_lookup = {(r["guide"], r["pos"]): r for r in annotated}
        _top10 = scored[:10]
        _rows_html = ""
        for _rank, (_guide, _pos) in enumerate(_top10, 1):
            _sc = score_guide(_pos, _guide, start_pos, end_pos)
            _ann = _ann_lookup.get((_guide, _pos), {})
            _gc = _ann.get("GC%", _gc_pct(_guide))
            _pam = _ann.get("PAM", "")
            _guide_span = f'<span style="font-family:monospace">{_guide}</span>'
            _pam_span = f'<span style="font-family:monospace;color:#EF9F27;font-weight:bold">{_pam}</span>'
            _seq_html = (_guide_span + _pam_span) if pam_side == "3'" else (_pam_span + _guide_span)
            _gc_val = float(str(_gc).replace("%", ""))
            _gc_html = (
                f'<div style="display:flex;align-items:center;gap:6px">'
                f'<div style="width:52px;height:4px;background:#1e2130;border-radius:2px">'
                f'<div style="width:{_gc_val:.0f}%;height:4px;background:#1D9E75;border-radius:2px"></div>'
                f'</div><span style="font-size:0.85em">{_gc_val:.0f}%</span></div>'
            )
            _sc_color = "#1D9E75" if _sc >= 0.8 else ("#EF9F27" if _sc >= 0.6 else "#993C1D")
            _sc_html = f'<span style="color:{_sc_color};font-weight:bold">{_sc:.3f}</span>'
            if _sc >= 0.8:
                _rb, _rf, _rl = "#085041", "#5DCAA5", "low"
            elif _sc >= 0.6:
                _rb, _rf, _rl = "#633806", "#FAC775", "med"
            else:
                _rb, _rf, _rl = "#6B1D1D", "#F09595", "high"
            _risk_html = (
                f'<span style="background:{_rb};color:{_rf};padding:2px 8px;'
                f'border-radius:4px;font-size:0.75em;font-weight:bold">{_rl}</span>'
            )
            _rows_html += (
                f'<tr style="border-bottom:1px solid #1e2130">'
                f'<td style="padding:8px 12px;color:#8899aa">{_rank}</td>'
                f'<td style="padding:8px 12px">{_seq_html}</td>'
                f'<td style="padding:8px 12px">{_gc_html}</td>'
                f'<td style="padding:8px 12px">{_sc_html}</td>'
                f'<td style="padding:8px 12px">{_risk_html}</td>'
                f'</tr>'
            )
        st.markdown(
            f'<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;'
            f'background:#151821;font-family:sans-serif;font-size:0.9em">'
            f'<thead><tr style="border-bottom:2px solid #1e2130;color:#8899aa;text-align:left">'
            f'<th style="padding:8px 12px">Rank</th>'
            f'<th style="padding:8px 12px">Sequence</th>'
            f'<th style="padding:8px 12px">GC%</th>'
            f'<th style="padding:8px 12px">Score</th>'
            f'<th style="padding:8px 12px">Risk</th>'
        ...
        if nav_click:
            ...
            if "win_pending" not in st.session_state:
                st.session_state.win_pending = None
            if nav_click == "start":
                st.session_state.win_pending = (0, min(curw, len(seq)))
            elif nav_click == "zin":
                neww = max(50, curw // 2); a = max(0, mid - neww//2); b = min(len(seq), a + neww); st.session_state.win_pending = (a, b)
            elif nav_click == "zout":
                neww = min(len(seq), curw * 2); a = max(0, mid - neww//2); b = min(len(seq), a + neww); st.session_state.win_pending = (a, b)
            elif nav_click == "center":
                a = max(0, mid - curw//2); b = min(len(seq), a + curw); st.session_state.win_pending = (a, b)
            elif nav_click == "end":
                a = max(0, len(seq) - curw); st.session_state.win_pending = (a, len(seq))
            elif nav_click == "1kb":
                w = 1000; a = max(0, mid - w//2); b = min(len(seq), a + w); st.session_state.win_pending = (a, b)
            st.rerun()
    ...
    with tabtrans:
        ...
        # ---------- Mutation Effects ----------
        with tabmut:
            st.subheader("Mutation Effects (codon-level, simple)")
            st.caption(
                "Toy viewer: pick a region, change one base, see the codon + amino acid change. DNA stays with A/C/G/T, no U.")
            ...
            with colL:
                region_mode = st.radio(
                    "Region",
                    ["Use current window", "Custom range"],
                    horizontal=True,
                    key="mut_region_mode",
                )

                if region_mode == "Use current window":
                    region_start = int(start_pos)
                    region_end = int(end_pos)
                else:
                    region_start = st.number_input(
                        "Start (0-based, inclusive)",
                        min_value=0,
                        max_value=max(0, len(seq) - 1),
                        value=int(start_pos),
                        step=1,
                        key="mut_start",
                    )
                ...
                if region_end <= region_start:
                    ...
                    st.markdown("**Selected DNA region (first 120 bp shown):**")
                    preview = region[:120] + ("..." if len(region) > 120 else "")
                    st.code(preview)

                    st.markdown("### Define a single-base mutation")
                    max_rel = max(1, len(region))
                    pos_rel = st.number_input(
                        "Position inside region (1-based)",
                        min_value=1,
                        max_value=max_rel,
                        value=1,
                        step=1,
                    ...
                    if st.button("Simulate mutation", type="primary", key="mut_go"):
                        if new_base not in {"A", "C", "G", "T"}:
                            st.error("New base must be one of A, C, G, T.")
                        else:
                            idx = int(pos_rel) - 1  # 0-based inside region
                            if idx < 0 or idx >= len(region):
                                st.error("Position out of range for this region.")
                            else:
                                # Build mutated region
                                region_mut = region[:idx] + new_base + region[idx + 1:]

                                # Codon index (0-based) inside the region
                                codon_index = idx // 3  # which codon
                                codon_pos_in_region = codon_index * 3
                                codon_global_start = region_start + codon_pos_in_region

                                orig_codon = region[codon_pos_in_region: codon_pos_in_region + 3]
                                mut_codon = region_mut[codon_pos_in_region: codon_pos_in_region + 3]

                                # Translate whole region, then look at the codon’s AA
                                aa_ref_seq = translate_dna(region, frame=0)
                                aa_mut_seq = translate_dna(region_mut, frame=0)

                                aa_ref = aa_ref_seq[codon_index] if codon_index < len(aa_ref_seq) else "?"
                                aa_mut = aa_mut_seq[codon_index] if codon_index < len(aa_mut_seq) else "?"


                                # Simple classification
                                def classify_effect(a0, a1):
                                    if a0 == a1:
                                        return "Silent (same amino acid)"
                                    if a0 == "*" and a1 != "*":
                                        return "Readthrough (stop → AA)"
                                    if a0 != "*" and a1 == "*":
                                        return "Nonsense (AA → stop)"
                                    return "Missense (AA change)"


                                effect = classify_effect(aa_ref, aa_mut)

                                # tRNA-ish anticodon (DNA style, so uses T not U)
                                anticodon_ref = reverse_complement(orig_codon)
                                anticodon_mut = reverse_complement(mut_codon)

                                # Show result table
                                st.markdown("### Result")
                                st.markdown(
                                    f"- **Genomic position (0-based):** {codon_global_start}–{codon_global_start + 3}\n"
                                    f"- **Region index (codon # inside region):** {codon_index + 1}"
                                )
                                ...
                                df_mut = pd.DataFrame(
                                    [
                                        {
                                            "Codon type": "Original",
                                            "Position in region (1-based)": codon_pos_in_region + 1,
                                            "DNA codon": orig_codon,
                                            "Anticodon (DNA-style)": anticodon_ref,
                                            "Amino acid": aa_ref,
                                        },
                                        {
                                            "Codon type": "Mutated",
                                            "Position in region (1-based)": codon_pos_in_region + 1,
                                            "DNA codon": mut_codon,
                                            "Anticodon (DNA-style)": anticodon_mut,
                                            "Amino acid": aa_mut,
                                        },
                                    ]
                                )
            ...
            with colR:
                st.markdown("### What this is showing")
                st.write("- You choose a DNA **region** (current window or custom).")
                st.write("- You change **one base** at a specific position.")
                st.write("- The app figures out which **codon** that base belongs to.")
                st.write("- Then it compares the original vs mutated **codon** and **amino acid**.")
                st.write("- Classification is only at the codon level: silent, missense, nonsense, readthrough.")
                st.caption("Everything is DNA-style (A/C/G/T). No U, no RNA alphabet here.")
...
# ==============================================================================
with tabRNA:
    st.subheader("RNA Tools — mRNA & tRNA from current DNA window")
    st.caption("Takes the current DNA window, builds mRNA (with U), tRNA anticodons, and amino acids.")

    # Use the current DNA window from the DNA controls
    region = seq[start_pos:end_pos]

    if len(region) < 3:
        st.info("Increase the DNA window (sidebar slider): need at least 3 bases to form one codon.")
    else:
        # Only keep full codons
        usable_len = (len(region) // 3) * 3
        region = region[:usable_len]

        max_codons = st.slider(
            "How many codons to display (from window start)",
            1,
            max(1, usable_len // 3),
            min(30, usable_len // 3),
            1,
            key="rna_tools_max_codons",
        )

        def _dna_to_mrna(dna: str) -> str:
            """DNA (A/C/G/T) → mRNA (A/C/G/U), same orientation."""
            return (dna or "").upper().replace("T", "U")

        def _mrna_to_trna_anticodon(mrna_codon: str) -> str:
            """
            mRNA codon (5'→3', with U) → tRNA anticodon (3'→5', with U).
            Implementation: convert U→T, use reverse_complement (DNA), then T→U back.
            """
            dna_like = (mrna_codon or "").upper().replace("U", "T")
            anti_dna = reverse_complement(dna_like)  # imported from helix_core.crisprutils
            return anti_dna.replace("T", "U")

        rows = []
        for i in range(0, min(usable_len, max_codons * 3), 3):
            dna_codon = region[i:i+3]  # A/C/G/T
            mrna_codon = _dna_to_mrna(dna_codon)  # A/C/G/U
            trna_anticodon = _mrna_to_trna_anticodon(mrna_codon)  # complementary with U

            # translate_dna works on DNA (with T)
            aa = translate_dna(dna_codon, frame=0)
            aa_one = aa[0] if aa else "?"

            rows.append({
                "Codon # (in window)": (i // 3) + 1,
                "Genomic start (bp)": start_pos + i,
                "DNA codon (5'→3')": dna_codon,
                "mRNA codon (5'→3', U)": mrna_codon,
                "tRNA anticodon (3'→5', U)": trna_anticodon,
                "Amino acid (1-letter)": aa_one,
            })

        import pandas as pd
        df_rna = pd.DataFrame(rows)
        st.dataframe(df_rna, use_container_width=True, hide_index=True)

        st.caption(
            "- DNA stays A/C/G/T (consistent with CRISPR/gRNA tools).\n"
            "- mRNA switches T → U.\n"
            "- tRNA anticodon is the complementary triplet to the mRNA codon, also with U.\n"
            "- Amino acid uses your existing translate_dna() logic on the DNA codon."
        )
