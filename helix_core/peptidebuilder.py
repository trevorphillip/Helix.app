...
def build_peptide_pdb_segmented(
    ...
    return header + out
"""Compatibility proxy for legacy module with optional dependency handling."""
...
_spec = importlib.util.find_spec("PeptideBuilder")
if _spec is None:
    def _missing(*_args: Any, **_kwargs: Any) -> None:
        raise RuntimeError("PeptideBuilder is required for peptide building features.")

    build_peptide_pdb = _missing  # type: ignore[assignment]
    build_peptide_pdb_segmented = _missing  # type: ignore[assignment]
    __all__ = ["build_peptide_pdb", "build_peptide_pdb_segmented"]
else:
    export("peptidebuilder", globals())