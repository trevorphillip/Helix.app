...
try:
    ...
    PLOTLY_TEMPLATE = {"layout": dict()}  # safe fallback
...
def plot_orf_map(sequence, orfs, start_pos=0, end_pos=None, min_aa=30):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=[start_pos, end_pos], y=[0, 0], mode="lines",
    ...
    for o in orfs:
        if o["end"] <= start_pos or o["start"] >= end_pos:
            continue
        ...
        fig.add_trace(go.Scatter(x=[x0, x1], y=[lane_y, lane_y], mode="lines",
                                 line=dict(color=color, width=10, shape="hv"),
                                 name=f"ORF {strand}", showlegend=False,
                                 hovertemplate=f"ORF ({strand})<br>Frame: {frame}<br>bp: {o['start']}-{o['end']}<br>aa: {aa}<extra></extra>"))
    fig.update_layout(title=f"ORF Map (min {min_aa} aa)", height=360,
...
def plot_overview_minimap(sequence, pam_sites, grnas, gc_x, gc_y):
    fig = go.Figure()
    if gc_x and gc_y:
        fig.add_trace(go.Scatter(x=gc_x, y=gc_y, mode="lines",
                                 line=dict(width=2, color=PALETTE["gc"]),
                                 name="GC% (windowed)",
                                 hovertemplate="Pos %{x:.0f}<br>GC %{y:.1f}%<extra></extra>"))
    if pam_sites:
        fig.add_trace(go.Scatter(x=pam_sites, y=[-2]*len(pam_sites), mode="markers",
    ...
    if grnas:
        ...
        fig.add_trace(go.Scatter(x=starts, y=[-4]*len(starts), mode="markers",
...
def plot_detail_map(
    ...
    fig.add_trace(go.Scatter(x=[start_pos, end_pos], y=[y_dna, y_dna], mode="lines",
                             line=dict(color=PALETTE["dna"], width=3), name="DNA", hoverinfo="skip"))

    for p in pam_sites:
        seq3 = sequence[p:p+3]
        fig.add_trace(go.Scatter(x=[p, p], y=[y_pam-0.2, y_pam+0.2], mode="lines",
                                 line=dict(color=PALETTE["pam"], width=3), name="PAM", showlegend=False,
                                 hovertemplate=f"PAM at {p}<br>Seq: {seq3}<extra></extra>"))

    for idx, (guide, pos) in enumerate(grnas, start=1):
        x0, x1 = pos, pos + guide_len
        gc = round((guide.count("G") + guide.count("C"))/max(1, len(guide)) * 100, 1)
        width = 12 if pos in highlight_positions else 8
        fig.add_trace(go.Scatter(x=[x0, x1], y=[y_grna, y_grna], mode="lines",
                                 line=dict(color=PALETTE["grna"], width=width),
                                 name="gRNA", showlegend=False,
                                 hovertemplate=f"gRNA #{idx}<br>Pos: {x0}-{x1}<br>GC: {gc}%<br>Seq: {guide}<extra></extra>"))
        fig.add_trace(go.Scatter(x=[x0], y=[y_grna], mode="markers",
    ...
    for ot in off_targets:
        ...
        fig.add_trace(go.Scatter(x=[s, e], y=[y_off, y_off], mode="lines",
                                 line=dict(color=PALETTE["off"], width=6, dash="dot"),
...
# ---------- Simple 3D double helix ----------
def plot_double_helix_windowed(sequence, pam_sites, grnas, start_pos=0, end_pos=None,
                               pam_len=3, guide_len=20, connector_step=1):
    seq_len = len(sequence)
    end_pos = min(seq_len, end_pos or seq_len)
    start_pos = max(0, min(start_pos, end_pos-1))
    ...
    def add_segment(i0, i1, color, name, hover):
        ...
        fig.add_trace(go.Scatter3d(x=x1[i0:i1+1], y=y1[i0:i1+1], z=z[i0:i1+1],
    ...
    for pam_abs in pam_sites:
        ...
        if 0 <= rel < n:
            i0, i1 = rel, min(n-1, rel + (pam_len - 1))
            pam_seq = sequence[start_pos+rel : start_pos+i1+1]
            add_segment(i0, i1, "#F87171", "PAM", f"PAM {pam_seq} at {pam_abs}")

    for guide, gpos_abs in grnas:
        rel = gpos_abs - start_pos
        if 0 <= rel < n:
            i0, i1 = rel, min(n-1, rel + (guide_len - 1))
            add_segment(i0, i1, "#34D399", "gRNA", f"gRNA ({guide_len}nt) at {gpos_abs}<br>Seq: {guide}")

    fig.update_layout(title=f"3D Double Helix (bp {start_pos}–{end_pos})",
...
def plot_motif_track(sequence, motifs, start_pos=0, end_pos=None):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=[start_pos, end_pos], y=[0, 0], mode="lines", line=dict(width=3),
                             name="DNA", hoverinfo="skip"))
    for m in items:
        color = "#60A5FA" if m.get("type") == "motif" else "#F59E0B"
        fig.add_trace(go.Scatter(x=[m["start"], m["start"]], y=[-0.4, 0.8], mode="lines",
                                 line=dict(color=color, width=3),
                                 hovertemplate=f"{m['name']}<br>Pattern: {m.get('pattern','')}<br>Start: {m['start']}<extra></extra>",
                                 showlegend=False))
    fig.update_layout(title="Motifs & Restriction Sites", height=220,
                      xaxis=dict(range=[start_pos, end_pos], title="Position (bp)"),
                      yaxis=dict(visible=False),
